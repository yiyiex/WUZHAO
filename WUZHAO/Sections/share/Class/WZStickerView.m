...
#define kStickerViewControlSize 28.0



@interface WZStickerView ()

@property (nonatomic, strong) WZStickerBorderView *borderView;
...
- (void)deleteTap:(UIGestureRecognizer *)recognizer
{
    ...
    else if ([recognizer state] == UIGestureRecognizerStateChanged)
    {
        ...
        if (self.bounds.size.width < self.minWidth || self.bounds.size.height < self.minHeight)
        {
            self.bounds = CGRectMake(self.bounds.origin.x,
                                     ...
                                     self.minWidth+1,
                                     self.minHeight+1);
            self.resizingControl.frame =CGRectMake(self.bounds.size.width-kStickerViewControlSize,
                                                   self.bounds.size.height-kStickerViewControlSize,
                                                   kStickerViewControlSize,
                                                   kStickerViewControlSize);
            self.deleteControl.frame = CGRectMake(0, 0,
                                                  kStickerViewControlSize, kStickerViewControlSize);
            self.customControl.frame =CGRectMake(self.bounds.size.width-kStickerViewControlSize,
                                                 0,
                                                 kStickerViewControlSize,
                                                 kStickerViewControlSize);
            self.prevPoint = [recognizer locationInView:self];
        ...
        self.borderView.frame = CGRectInset(self.bounds, kUserResizableViewGlobalInset, kUserResizableViewGlobalInset);
...
{
    self.borderView = [[WZStickerBorderView alloc] initWithFrame:CGRectInset(self.bounds, kUserResizableViewGlobalInset, kUserResizableViewGlobalInset)];
    ...
    self.deleteControl.image = [UIImage imageNamed:@"stickView_close"];
    ...
    self.resizingControl = [[UIImageView alloc]initWithFrame:CGRectMake(self.frame.size.width-kStickerViewControlSize,
                                                                        self.frame.size.height-kStickerViewControlSize,
                                                                        kStickerViewControlSize, kStickerViewControlSize)];
    self.resizingControl.backgroundColor = [UIColor clearColor];
    self.resizingControl.userInteractionEnabled = YES;
    self.resizingControl.image = [UIImage imageNamed:@"stickView_resize"];
    ...
    self.customControl = [[UIImageView alloc]initWithFrame:CGRectMake(self.frame.size.width-kStickerViewControlSize,
                                                                      0,
                                                                      kStickerViewControlSize, kStickerViewControlSize)];
...
- (void)setButton:(WZSTICKERVIEW_BUTTONS)type image:(UIImage*)image
{
    switch (type)
    {
        case WZSTICKERVIEW_BUTTON_RESIZE:
            self.resizingControl.image = image;
            break;
        case WZSTICKERVIEW_BUTTON_DEL:
            self.deleteControl.image = image;
            break;
        case WZSTICKERVIEW_BUTTON_CUSTOM:
