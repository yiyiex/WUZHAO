...
-(void)handleTapGesture:(UIGestureRecognizer*)gestureRecognizer{
    ...
    CGPoint locationInScrollView = [gestureRecognizer locationInView:self.self.scrollView];
...
- (void)layoutSubviews{
    ...
    if (_needsReload) {
        ...
        // 重置_self.scrollView的contentSize
        switch (orientation) {
            ...
            case PagedFlowViewOrientationVertical:{
                self.scrollView.frame = CGRectMake(0, 0, _pageSize.width, _pageSize.height);
                self.scrollView.contentSize = CGSizeMake(_pageSize.width ,_pageSize.height * _pageCount);
                CGPoint theCenter = CGPointMake(CGRectGetMidX(self.bounds), CGRectGetMidY(self.bounds));
                self.scrollView.center = theCenter;
                break;
            }
    ...
    [self setPagesAtContentOffset:self.scrollView.contentOffset];//根据当前self.scrollView的offset设置cell
...
- (void)scrollToPage:(NSUInteger)pageNumber {
    if (pageNumber < _pageCount) {
        switch (orientation) {
            case PagedFlowViewOrientationHorizontal:
                [self.scrollView setContentOffset:CGPointMake(_pageSize.width * pageNumber, 0) animated:YES];
                break;
            case PagedFlowViewOrientationVertical:
                [self.scrollView setContentOffset:CGPointMake(0, _pageSize.height * pageNumber) animated:YES];
                break;
        }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    switch (orientation) {
        case PagedFlowViewOrientationHorizontal:
            pageIndex = floor(self.scrollView.contentOffset.x / _pageSize.width);
            break;
        case PagedFlowViewOrientationVertical:
            pageIndex = floor(self.scrollView.contentOffset.y / _pageSize.height);
            break;
        default:
            break;
    }
