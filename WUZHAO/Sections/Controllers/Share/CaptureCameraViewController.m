...
@interface CaptureCameraViewController ()
{
    int alphaTimes;
    CGPoint currTouchPoint;
}

@property (nonatomic)  CaptureSessionManager *captureManager;
...
@implementation CaptureCameraViewController
...
- (void)addTopViewWithText:(NSString*)text {
    if (!_topContainerView) {
        CGRect topFrame = CGRectMake(0, 0, WZ_APP_SIZE.width, CAMERA_TOPVIEW_HEIGHT);
...
- (void)addbottomContainerView {
    ...
    CGRect bottomFrame = CGRectMake(0, bottomY, WZ_APP_SIZE.width, WZ_APP_SIZE.height - bottomY);
...
- (void)addCameraMenuView {
    ...
    //拍照按钮
    CGFloat downH = (isHigherThaniPhone4_WZ ? CAMERA_MENU_VIEW_HEIGH : 0);
    ...
    CGFloat menuViewY = (isHigherThaniPhone4_WZ ? WZ_DEVICE_SIZE.height - CAMERA_MENU_VIEW_HEIGH : 0);
    UIView *menuView = [[UIView alloc] initWithFrame:CGRectMake(0, menuViewY, self.view.frame.size.width, CAMERA_MENU_VIEW_HEIGH)];
    menuView.backgroundColor = (isHigherThaniPhone4_WZ ? bottomContainerView_DOWN_COLOR : [UIColor clearColor]);
...
- (void)addMenuViewButtons {
    ...
    for (int i = 0; i < actionArr.count; i++) {
        ...
        CGFloat theH = (!isHigherThaniPhone4_WZ && i == 0 ? _bottomContainerView.frame.size.height : CAMERA_MENU_VIEW_HEIGH);
        UIView *parent = (!isHigherThaniPhone4_WZ && i == 0 ? _bottomContainerView : _cameraMenuView);
...
- (void)showCameraCover:(BOOL)toShow {
    ...
    [UIView animateWithDuration:0.38f animations:^{
        ...
        downFrame.origin.y = (toShow ? WZ_APP_SIZE.width / 2 + CAMERA_TOPVIEW_HEIGHT : _bottomContainerView.frame.origin.y);
...
- (void)takePictureBtnPressed:(UIButton*)sender {
    ...
    [_captureManager takePicture:^(UIImage *stillImage) {
        ...
        //your code 0
        SCNavigationController *nav = (SCNavigationController*)weakSelf_WZ.navigationController;
